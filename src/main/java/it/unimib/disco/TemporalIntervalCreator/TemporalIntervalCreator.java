...
public static void main (String []args) throws FileNotFoundException{
	if (args.length < 1) {
		...
		// Read temporalDefacto facts
				...
				List<String> yagoFacts = ReadFiles.getURIs(new File(TemporalIntervalCreator.class.getResource(args[2]).getFile()));
				...
				FileOutputStream fos1 = new FileOutputStream("evaluationResultComplete.csv");
				...
				for (String uri: reducedMatrix2.keySet()){
							//DateOccurrence [][] rm = reducedMatrix2.get(uri);
							...
							for (String obj: objYearOccu.keySet()){
								...
								odc.put(obj, ta.dcCalculator(yearOccs,matrixManhattanDuration));
			    ...
			    for (String uri:sodc.keySet()){
					...
					tempodefactoIntervalsUri.put(uri,sel.selection(objIntervals));
				...
				try {
					...
					BufferedWriter bw = new BufferedWriter(new FileWriter(new File("evaluationResult.csv")));
